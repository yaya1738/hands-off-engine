...
Consumes type=task_request entries from the canonical coordination bus
(ai/coordination/messages.jsonl), validates sender and request contract,
deduplicates by msg_id, and persists admission decisions durably.

Design constraints (per Factory directive #284-13):
...
# Bounded allowed request types (must match task_request.py ALLOWED_TYPES)
ALLOWED_REQUEST_TYPES = frozenset({"work_request", "question", "status_probe"})

# Registered parties that may issue task_requests
# Only agent-role parties with trust >= 7 are allowed
ALLOWED_SENDERS = frozenset({"anyclaw", "openclaw"})

# Max message length to prevent bus pollution
...
def _fingerprint(msg: dict) -> str:
    """Deterministic fingerprint from msg_id for dedup."""
...
class RequestIntake:
    ...
    Reads task_request messages from canonical bus, validates sender/type,
    deduplicates by msg_id, and persists admission decisions.
    """
    ...
    def _validate_request_type(self, msg: dict) -> Tuple[bool, str]:
        """Validate request type is in the bounded allowed set."""
    ...
    def _validate_message(self, msg: dict) -> Tuple[bool, str]:
        ...
        if not msg.get("type") or msg["type"] != "task_request":
    ...
    def _validate_contract(self, msg: dict) -> Tuple[bool, str]:
        """Full contract validation: sender + type + envelope."""
        ...
        if not ok:
            return False, reason
        ok, reason = self._validate_request_type(msg)
        if not ok:
            return False, reason
        return True, "ok"

    def _read_task_requests(self) -> List[dict]:
        """Read task_request messages from the canonical bus, including governed CommHub envelopes."""
    ...
    def admit_once(self) -> List[dict]:
        """Process all pending task_request messages: validate, dedup, admit or reject.

        Returns list of admission decisions (both admitted and rejected).
        """
    ...
    def status(self) -> dict:
        ...
        return {
            ...
            "last_admission": (
                self.state.get("admissions", [])[-1]
                if self.state.get("admissions") else None
            ),
        }

    def get_admitted(self) -> List[dict]:
        """Return all admitted request decisions."""
        return self.state.get("admissions", [])

    def get_rejections(self) -> List[dict]:
        """Return all rejection decisions."""
