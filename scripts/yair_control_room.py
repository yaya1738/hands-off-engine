...
#feed{display:flex;flex-direction:column;gap:7px;max-height:62vh;overflow:auto}.msg{background:#11182a;border:1px solid #26304a;border-radius:9px;padding:9px}.meta{color:#8190ad;font-size:11px}.body{white-space:pre-wrap;margin-top:4px;word-break:break-word}
...
<main><section class="grid"><div class="card"><div class="label">Bus</div><div id="bus" class="value">—</div></div><div class="card"><div class="label">Messages</div><div id="count" class="value">0</div></div><div class="card"><div class="label">AI parties</div><div id="parties" class="value">—</div></div><div class="card"><div class="label">Last activity</div><div id="last" class="value">—</div></div></section>
<section id="feed"></section><form id="send"><input id="text" autocomplete="off" placeholder="Tell the AI system what to work on…"><button>Send</button></form></main>
<script>
const esc=s=>String(s??'').replace(/[&<>\"]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;'}[c]));
async function refresh(){try{let r=await fetch('/api/events?limit=120');let d=await r.json();document.querySelector('#bus').textContent=d.available?'ONLINE':'WAITING';document.querySelector('#count').textContent=d.events.length;let ps=[...new Set(d.events.flatMap(x=>[x.from,x.to]).filter(Boolean))];document.querySelector('#parties').textContent=ps.length;document.querySelector('#last').textContent=d.events.length?new Date(d.events[d.events.length-1].timestamp).toLocaleTimeString():'—';document.querySelector('#feed').innerHTML=d.events.slice().reverse().map(x=>`<article class="msg"><div class="meta">${esc(x.timestamp)} · ${esc(x.from)} → ${esc(x.to)} · ${esc(x.type)}</div><div class="body">${esc(JSON.stringify(x.payload??x.message??x,null,2))}</div></article>`).join('')}catch(e){document.querySelector('#bus').textContent='ERROR'}}
