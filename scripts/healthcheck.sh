...
# Skip if called from orchestrator (prevents circular dependency that causes 60s timeout)
if [ -z "$HEALTHCHECK_FROM_ORCHESTRATOR" ]; then
    ...
    echo "[$(date)] Skipping orchestrator (called from orchestrator context)"
fi
