...
class CommHub:
    ...
    def register_party(self, party_id, name, role, channels, trust_level=5, info_scope="structured", notify_on=None, fmt="json"):
        self.parties[party_id] = {"id": party_id, "name": name, "role": role, "trust_level": trust_level, "channels": channels, "info_scope": info_scope, "notify_on": notify_on or ["system_status"], "format": fmt}
        self._save_parties(); log.info(f"Registered party: {party_id} ({name}, role={role})")
    ...
    def receive(self, sender_id, msg_type, payload, channel="webhook"):
        ...
        msg = {"id": str(uuid.uuid4()), "from": sender_id, "to": "system_internal", "type": msg_type, "channel": channel, "timestamp": datetime.now(timezone.utc).isoformat(), "payload": payload, "sender_trust": sender.get("trust_level", 0), "sender_role": sender.get("role", "unknown")}
        self._log_inbound(msg)
        bus_msg = dict(msg); bus_msg["type"] = f"inbound_from_{sender.get('role', 'external')}"; self._write_to_messages_jsonl(bus_msg)
    ...
    def _deliver_telegram(self, msg, party_id):
        ...
        try:
            subprocess.run(["termux-notification", "-t", str(title)[:50], "-c", str(text)[:200], "--id", str(msg.get("id", ""))[:10]], capture_output=True, timeout=5)
    ...
    def _deliver_notification(self, msg, party_id):
        ...
        try:
            subprocess.run(["termux-notification", "-t", str(title)[:50], "-c", body, "--id", str(msg.get("id", ""))[:10]], capture_output=True, timeout=5)
    ...
    def _handle_approval_response(self, msg):
        ...
        try:
            ...
            sc = SystemControl(repo_root=self.repo_root); result = sc.approve(cmd_id) if action == "approve" else sc.reject(cmd_id)
    ...
    def _handle_status_query(self, msg):
        ...
        for name in ("autonomy_status.json", "hands_off_brain.json"):
            ...
            if p.exists():
                try:
                    data = json.loads(p.read_text()); status["brain" if name.startswith("hands") else "status"] = data
                except Exception: pass
    ...
    def _handle_trade_command(self, msg):
        ...
        try:
            from scripts.control_api import SystemControl
            sc = SystemControl(repo_root=self.repo_root); cmd_id = sc.execute(action=payload.get("action", "trade"), params=payload.get("params", {}), mode=payload.get("mode", "LIVE"), sync=False)
