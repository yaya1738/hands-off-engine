...
def call_github_copilot(
    ...
    if pending_response:
        return {
            ...
            "tokens": len(pending_response.split()) * 2  # Rough token estimate
        }
    ...
    try:
        with open(messages_file, 'r') as f:
            for line in f:
                if line.strip():
                    msg = json.loads(line)
                    if msg.get('from') == 'copilot' and msg.get('type') in ['response', 'info']:
...
def _queue_copilot_request(session_goal: str, prior_messages: List[Dict]) -> str:
    ...
    if prior_messages:
        ...
        context_summary = " | ".join([
            f"{m.get('from_agent', 'unknown')}: {m.get('content', '')[:100]}..."
            for m in last_few
        ])
