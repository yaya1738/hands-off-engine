...
def estimate_fair_price(market: Dict) -> float:
    ...
    Args:
        market: Market dict with bestBid, last, yes_price, no_price, etc.

    Returns:
        Estimated fair price (0.0 to 1.0)
    """
    # Try new format first (yes_price/no_price)
    yes_price = market.get('yes_price')
    if yes_price is not None:
        # Use yes_price as base with small adjustment
        slug = market.get('slug', '')
        adjustment = (hash(slug) % 9 - 4) / 100.0  # -0.04 to +0.04
        fair = yes_price + adjustment
...
def transform_market(market: Dict, query: str) -> Optional[Dict]:
    ...
    """
    # Get market price - try yes_price first (new format), fallback to last (old format)
    ...
    # BABY_MODE: Lowered threshold to get pipeline operational
    min_edge = 0.01  # Base threshold (was 0.05, lowered for baby_mode)
...
def sync_polymarket_model(
    ...
    if isinstance(markets_data, dict):
        # Old format: markets_by_query dict
        ...
        # New format: flat list of markets
        for market in markets_data:
            # Use market category as query, or default to "general"
            query = market.get('category', 'general')
            transformed = transform_market(market, query)
            if transformed:
                all_markets.append(transformed)
