...
def _import_modules(path: Path) -> set[str]:
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Import): modules.update(alias.name.casefold() for alias in node.names)
        elif isinstance(node, ast.ImportFrom) and node.module: modules.add(node.module.casefold())
    return modules
...
def test_legacy_claude_processor_is_not_a_model_consumer():
    ...
    assert "exec /usr/bin/python3" in source and "autonomy_liveness_supervisor.py" in source
...
def test_external_control_surface_exists():
    ...
    assert "HumanLoop" in listener and "getUpdates" in listener and "ALLOWED_CHAT_ID" in listener

def test_ai_nexus_runner_does_not_eagerly_import_chatgpt():
    assert "ai_nexus.provider_chatgpt" not in _import_modules(REPO_ROOT / "ai_nexus" / "runner.py")
