...
def test_selects_highest_value_candidate_with_stable_tie_breaking():
    ...
    result = loop.select_next({"gaps": ["zeta gap", "alpha gap"], "findings": ["lower priority finding"]})
...
def test_missing_integration_boundary_outranks_generic_gap():
    loop = FactoryAutonomousObjectiveLoop()
    result = loop.select_next({"gaps": ["generic capability gap"], "discovery": {"missing": ["improvement_executor"]}})
    assert result["selected"]["source"] == "integration_health"
    assert "improvement_executor" in result["selected"]["objective"]


def test_structured_gap_becomes_actionable_objective_instead_of_being_dropped():
    loop = FactoryAutonomousObjectiveLoop()
    result = loop.select_next({"phase": "pre_dass", "strategic_objective": "maximize verified progress toward DASS", "gaps": [{"capability": "FactoryAuthorityGateway", "component": "execution verification"}]})
    selected = result["selected"]
    assert selected["source"] == "capability_gap"
    assert selected["objective"] == "FactoryAuthorityGateway"


def test_pre_dass_gap_outranks_broad_strategic_objective():
    loop = FactoryAutonomousObjectiveLoop()
    strategic = "maximize verified progress toward DASS"
    result = loop.select_next({"phase": "pre_dass", "strategic_objective": strategic, "gaps": ["close the remaining DASS production gap"]})
    selected = result["selected"]
    assert selected["source"] == "capability_gap"
    assert selected["objective"] == "close the remaining DASS production gap"
    assert selected["priority_score"] > next(c["priority_score"] for c in result["candidates"] if c["source"] == "strategic_objective")


def test_post_dass_value_metadata_can_outrank_broad_strategic_objective():
    loop = FactoryAutonomousObjectiveLoop()
    result = loop.select_next({"phase": "post_dass", "strategic_objective": "maximize useful post-DASS operation indefinitely", "gaps": [{"objective": "high-value resilient capability", "expected_value": 1.0, "confidence": 1.0, "reversibility": 1.0, "cost": 0.0, "risk": 0.0}]})
...
def test_exhausted_strategic_objective_becomes_bounded_continuity_checkpoint():
    ...
    result = loop.select_next({"strategic_objective": objective, "excluded_objectives": [objective], "cycle_count": 7})
    selected = result["selected"]
    assert result["status"] == "selected"
...
def test_continuity_checkpoint_changes_with_cycle_count():
    loop = FactoryAutonomousObjectiveLoop()
    objective = "keep improving autonomous capability"
    first = loop.select_next({"strategic_objective": objective, "excluded_objectives": [objective], "cycle_count": 1})["selected"]["objective"]
    second = loop.select_next({"strategic_objective": objective, "excluded_objectives": [objective], "cycle_count": 2})["selected"]["objective"]
    assert first != second
...
def test_retired_continuity_checkpoint_advances_to_next_cycle():
    ...
    retired = "Perform bounded autonomous continuity checkpoint 1: inspect the governed runtime for the highest-value actionable capability gap, validate the finding through existing safety and authority gates, and record the next bounded improvement without weakening any safety, audit, cost, risk, or verification boundary."
    result = loop.select_next({"strategic_objective": strategic, "excluded_objectives": [strategic, retired], "cycle_count": 1})
    selected = result["selected"]
    assert result["status"] == "selected"
    assert selected["source"] == "autonomous_continuity"
