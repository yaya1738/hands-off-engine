...
class FactoryAutonomousObjectiveLoop:
    ...
    def discover_candidates(self, context: Dict[str, Any] | None = None) -> List[Dict[str, Any]]:
        ...
        if strategic:
            ...
            if objective:
                candidates.append({"objective": objective, "source": "strategic_objective", "score": 100, "reason": "persistent strategic objective"})
        for gap in context.get("gaps", []):
            objective = self._text(gap)
            if objective:
                item = {"objective": objective, "source": "capability_gap", "score": 90, "reason": "observed capability gap"}
        ...
        if isinstance(discovery, dict) and discovery.get("missing"):
            for missing in discovery["missing"]:
                ...
                candidates.append({"objective": objective, "source": "integration_health", "score": 95, "reason": "required autonomous component is missing"})
        for finding in context.get("findings", []):
            objective = self._text(finding)
            if objective:
                item = {"objective": objective, "source": "runtime_finding", "score": 80, "reason": "runtime finding requires follow-up"}
    ...
    def _phase_score(self, candidate: Dict[str, Any], phase: str | None, has_actionable: bool) -> float:
        ...
        if phase == "pre_dass":
            return base + {"integration_health": 20, "capability_gap": 15, "runtime_finding": 10, "autonomous_continuity": 5, "strategic_objective": -30 if has_actionable else 0}.get(source, 0)
        if phase == "post_dass":
            source_bonus = {"integration_health": 15, "capability_gap": 10, "runtime_finding": 5, "autonomous_continuity": 0, "strategic_objective": -35 if has_actionable else 0}.get(source, 0)
    ...
    def prioritize(self, candidates: Iterable[Dict[str, Any]], excluded_objectives: Iterable[str] | None = None, cycle_count: int = 0, phase: str | None = None) -> Dict[str, Any] | None:
        ...
        has_actionable = any(candidate.get("source") in {"capability_gap", "integration_health", "runtime_finding"} for candidate in candidate_list if isinstance(candidate, dict))
    ...
    def select_next(self, context: Dict[str, Any] | None = None) -> Dict[str, Any]:
        ...
        selected = self.prioritize(candidates, context.get("excluded_objectives", []), int(context.get("cycle_count", 0) or 0), context.get("phase"))
        result = {"status": "selected" if selected else "no_candidate", "candidate_count": len(candidates), "candidates": candidates, "excluded_objectives": list(context.get("excluded_objectives", [])), "phase": context.get("phase"), "selected": selected}
