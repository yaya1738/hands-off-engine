...
# Token ID cache to avoid repeated API calls
_TOKEN_ID_CACHE = {}

def resolve_token_id(market_slug: str, side: str) -> Optional[str]:
    """Resolve a market slug to its CLOB token ID.

    Args:
        market_slug: The market slug (e.g., 'will-2-fed-rate-cuts-happen-in-2025')
        side: 'YES' or 'NO' to get the corresponding token

    Returns:
        The numeric token ID string, or None if resolution fails
    """
    cache_key = f"{market_slug}:{side}"
    if cache_key in _TOKEN_ID_CACHE:
        return _TOKEN_ID_CACHE[cache_key]

    try:
        import requests
        r = requests.get(f'https://gamma-api.polymarket.com/markets?slug={market_slug}', timeout=10)
        if r.ok and r.json():
            ...
            if len(clob_ids) >= 2:
                ...
                LOG.info(f"Resolved {market_slug} {side} -> token {token_id[:20]}...")
                return token_id
    except Exception as e:
        LOG.warning(f"Token ID resolution failed for {market_slug}: {e}")
    return None
        ...
        for action in planned_actions:
            ...
            if executor_mode == "dryrun":
                ...
                if not live_trading_active or trader is None:
                    ...
                    trade_token_id = getattr(action, 'token_id', None)

                    # If no token_id, try to resolve from market slug
                    if not trade_token_id or not trade_token_id.isdigit():
                        resolved = resolve_token_id(action.market_id, action.side)
                        if resolved:
                            trade_token_id = resolved
                        else:
                            LOG.warning(f"Failed to resolve token ID for {action.market_id}")
                            result = ExecutionResult(
                                market_id=action.market_id,
                                market_name=action.market_name,
                                success=False,
                                message=f"BLOCKED: Could not resolve token ID for market",
                                executed_amount=0.0
                            )
                            results.append(result)
                            continue
                    ...
                    if action.amount < 1.0:
                        LOG.warning(f"🛑 Order too small: ${action.amount:.2f} < $1 minimum")
                        result = ExecutionResult(
                            market_id=action.market_id,
                            market_name=action.market_name,
                            success=False,
                            message=f"BLOCKED: Order ${action.amount:.2f} below $1 minimum",
                            executed_amount=0.0
                        )
                        results.append(result)
                        continue
