...
The next interaction layer builds on the existing Control Room projection rather than creating another channel.
...
- Correlation uses the existing task/message identifiers so an observer can follow a request through its lifecycle.

The purpose is not to give the UI more power. It is to make interaction observable, correlated, and governed end-to-end so humans and autonomous agents can participate in the same operational loop.
