...
state_path = os.path.expanduser("~/hands-off/data/finances/state.json")
...
# Build a block that uses ONLY classic formatting (no nested f-strings)
block = """
        ...
        if self.path == "/finance/networth":
            ...
            import os, json
            try:
                with open(os.path.expanduser("%(STATE)s"), "r", encoding="utf-8") as f:
                    st = json.load(f)
                bal = st.get("balances", {})
                total = 0.0
                items = []
        ...
        if self.path == "/metrics":
            ...
            return
        # --- end inserted block ---
""" % {"STATE": state_path.replace("\\", "\\\\")}

new = src[:idx] + block + src[idx:]
bak = fn + ".pre_networth_metrics"
